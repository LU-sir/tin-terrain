...
// http://resources.esri.com/help/9.3/arcgisdesktop/com/gp_toolref/spatial_analyst_tools/esri_ascii_raster_format.htm
bool write_raster_to_asc(const std::string &filename,
...
bool write_raster_to_asc(FileLike &file, const RasterDouble &raster) {
  ...
  */
  ...
  // file << "XLLCORNER "    << std::setprecision(9) << raster.getXPos() <<
  // std::endl;
  fmt::format_to(line_buffer, "XLLCORNER {:.9f}\n", raster.get_pos_x());
  // file << "YLLCORNER "    << std::setprecision(9) << raster.getYPos() <<
  // std::endl;
  fmt::format_to(line_buffer, "YLLCORNER {:.9f}\n", raster.get_pos_y());
  // file << "CELLSIZE "     << std::setprecision(9) << raster.getCellsize() <<
  // std::endl;
  fmt::format_to(line_buffer, "CELLSIZE {:.9f}\n", raster.get_cell_size());
  // file << "NODATA_VALUE " << std::setprecision(9) << raster.getNoDataValue()
  // << std::endl;
...
static bool get_transformation_matrix(GDALDataset *dataset,
  ...
  if ((fabs(gt.scale_x) - fabs(gt.scale_y)) > (fabs(gt.scale_x) / 1000)) {
    TNTN_LOG_ERROR("Can not process rasters with non square pixels ({}x{})",
                   fabs(gt.scale_x), fabs(gt.scale_y));
    return false;
  }
...
static bool is_valid_projection(GDALDataset *dataset) {
  ...
  if (projection_wkt == NULL) {
    TNTN_LOG_ERROR(
        "Input raster file does not provide spatial reference information");
    return false;
  }
  ...
  for (int i = 0; i < matches_number; i++) {
    if (web_mercator.IsSame(
            static_cast<OGRSpatialReference *>(matched_projections[i]))) {
      matched = true;
      break;
    }
  }
...
bool load_raster_file(const std::string &file_name, RasterDouble &target_raster,
                      bool validate_projection) {
  initialize_gdal_once();

  TNTN_LOG_INFO("Opening raster file {} with GDAL...", file_name);

  GDALDataset_ptr dataset(
      static_cast<GDALDataset *>(GDALOpen(file_name.c_str(), GA_ReadOnly)),
      &GDALClose_wrapper);
  ...
  if (bands_count == 0) {
    ...
    TNTN_LOG_WARN(
        "File {} has {} raster bands, processing with a raster band #1",
        file_name, bands_count);
  }
  ...
  if (raster_band->RasterIO(GF_Read, 0, 0, raster_width, raster_height,
                            target_raster.get_ptr(), raster_width,
                            raster_height, GDT_Float64, 0, 0) != CE_None) {
